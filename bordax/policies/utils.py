...
class Policy:
    init: Callable[..., Any]
    get_distribution: Callable[..., Tuple[distrax.DistributionLike, Dict]]
...
def action_value_factory(policy_value: PolicyValue):

    def make_policy(policy_params, deterministic=False):
        policy_mlp = policy_value.policy

        def get_action(obs, key):
            pi, info = policy_mlp.get_distribution(policy_params, obs)
            if deterministic:
                return pi.mode(), {"info": info}
            action, log_prob = pi.sample_and_log_prob(seed=key)
            return action, {"log_prob": log_prob, "info": info}
